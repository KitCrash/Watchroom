...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', ({ roomId, username = 'User' }) => {
    ...
    const sysMsg = db.addMessage(cleanId, {
      sender: 'system',
      text: `${username} joined the room. (${userCount} connected)`,
    });
    io.to(cleanId).emit('new-message', { message: sysMsg });
  });
  ...
  socket.on('disconnecting', () => {
    ...
    if (cleanId) {
      ...
      const sysMsg = db.addMessage(cleanId, {
        sender: 'system',
        text: `${socket.data.username || 'Partner'} left the room.`,
      });
      socket.to(cleanId).emit('new-message', { message: sysMsg });
    }
  });
});
